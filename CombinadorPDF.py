...
class FrameArquivos(tk.LabelFrame):
    ...
    def openFileDirectory(self):
        ...
        self.filename = filedialog.askopenfilenames(**opcoes) #filename é uma tupla que contém os caminhos dos arquivos selecionados

        self.CombinadorPDF.selectFiles(self.filename)

        #2 lugares precisarão desses arquivos
        #1) a clase combinadorPDF e a interface de arquivos selecionados
...
class FrameInserir(tk.LabelFrame):
    ...
    def __init__(self, master):
        ...
        self.master.FrameArquivos = FrameArquivos

        # opçoes teste
        self.pdfFiles =  #lista de arquivos selecionados pelo usuário
