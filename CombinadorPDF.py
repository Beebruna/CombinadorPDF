...
class CombinadorPDF:
    ...
    def adicionar_arquivos(self, nome_arquivos):
        ...
        # e fitz.document como value no dicionario pdf
...
class Pdf:
    def __init__(self, fitz_doc, nome_arquivo):
...
class MenuBar(tk.Menu):
    def __init__(self, parent, grandparent, frame_treeview):
        tk.Menu.__init__(self, parent)
        # não é possivel colocar menu diretamente no frame
        grandparent.config(menu=self)

        self.parent = parent
        self.frame_treeview = frame_treeview
        ...
        self.menu_arquivos.add_command(
            ...
            command=lambda: self.excluir_arquivos(self.frame_treeview.treeview.selection()))
        self.menu_arquivos.add_command(
            label='Excluir todos os arquivos',
            command=lambda: self.excluir_arquivos(self.frame_treeview.treeview.get_children()))
        self.menu_arquivos.add_command(
    ...
    def ordenar(self, ordenacao):
        ...
        nome_arquivos = list(self.parent.combinadorPDF.nome_arquivos) # evitar que apontem para o mesmo objeto
            ...
            while not sorted:
                ...
                for i in range(len(lista_nomes) - 1):
                    if lista_nomes[i].lower() > lista_nomes[i+1].lower():
                        ...
                        nome_arquivos[i], nome_arquivos[i+1] = nome_arquivos[i+1], nome_arquivos[i]
    ...
    def editar_arquivo(self):
        ...
        id_arquivo = self.frame_treeview.treeview.selection()[0]
        nome_arquivo = self.frame_treeview.treeview.item(id_arquivo, 'text')
        
        top = tk.Toplevel()
        top.title(nome_arquivo)
        
        self.frame_pdf = FrameVisualizadorPDF(top, 
                                              nome_arquivo, 
                                              self.parent.combinadorPDF.pdf[nome_arquivo])
    
    def download_pdfs(self):
        # combina pdfs e faz download
        ...
        # juntando pdfs
        pdf = fitz.open()
        for nome_arquivo in self.parent.combinadorPDF.nome_arquivos:
            pdf.insertPDF(self.parent.combinadorPDF.pdf[nome_arquivo])
...
class FrameTreeview(tk.Frame):
    def __init__(self, parent):
        ...
        # colocando alguns botoes em uma frame para organiza-los
    ...
    def adicionar_arquivos(self):
        # abre janela para seleção de arquivos
    ...
    def excluir_arquivos(self):
        # exclui arquivos selecionados da treeview
        id_arquivos = self.treeview.selection()

        # adiciona nomes a uma lista enquanto deleta da treeview
        nome_arquivos = []
        for id_arquivo in id_arquivos:
    ...
    def up(self):
        ...
        self.parent.combinadorPDF.reinserir_antes(nome_arquivo)
    ...
    def down(self):
        ...
        # move a posição do item na lista do objeto combinadorPDF
        nome_arquivo = self.treeview.item(id_arquivo, 'text')
        self.parent.combinadorPDF.reinserir_depois(nome_arquivo)
...
class FrameVisualizadorPDF(tk.Frame):
    def __init__(self, parent, nome_arquivo, pdf):
        tk.Frame.__init__(self, parent)
        self.parent = parent
        
        self.pdf = Pdf(pdf, nome_arquivo)
        ...
        self.label_imagem.imagem = self.pdf.lista_imagens[0] # manter segunda referencia
...
class FrameMetadados(tk.Frame):
    def __init__(self, parent, pdf):
        ...
        self.label_keywords = tk.Label(self.frame_keywords, text='Palavras-chave')
        ...
        self.label_mod_date = tk.Label(self.frame_mod_date, text='Data de Modificação')
...
class MainApplication(tk.Frame):
    def __init__(self, parent):
        ...
        self.menu_bar = MenuBar(self, parent, self.frame_treeview)
